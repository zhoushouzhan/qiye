...
class ListInfo extends Base
{
    ...
    public function getList(int $isremove = 0, $page = 1, $keyword = '')
    {
        ...
        if ($this->mod['type'] == 'classic') {
            ...
            if ($isremove) {
                $res = $this->table::onlyTrashed()->where($map)->order('id', 'desc')->paginate(30, false, ['page' => $page]);
            } else {
                $res = $this->table::order('id', 'desc')->where($map)->paginate(30, false, ['page' => $page]);
            }
