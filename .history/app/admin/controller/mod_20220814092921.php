 ...
 * @LastEditTime: 2022-08-07 10:14:24
...
class Mod extends Base
{
    ...
    {
        if ($name) {
            $r = \app\common\model\Mod::where()->find();
    ...
    //增加字段
    public function updateCol(int $id, array $colids = [], array $colalias)
    {
        if ($this->request->isPost()) {
            $r = \app\common\model\Mod::find($id);
            $r->cols = json_encode($colids);
            $r->colalias = json_encode($colalias);
            $r->save();
