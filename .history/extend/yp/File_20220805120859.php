...
class File
{
    ...
    public function __construct($data = [])
    {
        ...
        $this->cols = Db::name('cols')->column('*', 'id');
    }
    //创建模型文件
    ...
    public function createTraitFile()
    {
        ...
        foreach ($this->mod['colsid'] as $value) {
            $col = $this->cols[$value];
            $modFileCode .= $this->getColAttr($col);
        }
    ...
    public function getColAttr($col)
    {
        ...
        $comment = $col['comment'];
        $type = $col['formitem'];
        $defvalue = $col['defvalue'];
        $formvalue = $col['formvalue'];
        $attrCode = '';
        switch ($type) {
            case 'datetime':
                $attrCode .= "
//关联{$comment}
public function set{$name}Attr(\$value, \$data) {
...
//关联{$comment}
public function get{$name}Attr(\$value, \$data) {
	return \app\common\model\Classify::getSelected('{$defvalue}',\$value,\$data);
}
			";
