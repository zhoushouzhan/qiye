...
class Model
{
    ...
    public static function addCol($value, $dbname, $tbname)
    {
        ...
        switch ($value['type']) {
            case 'INT':
                $query = "ALTER TABLE {$tbname} ADD `{$value['name']}` INT({$value['length']}) SIGNED NOT NULL COMMENT '{$value['comment']}'";
                break;
            case 'FLOAT':
                $query = "ALTER TABLE {$tbname} ADD `{$value['name']}` FLOAT({$value['length']}) SIGNED NOT NULL COMMENT '{$value['comment']}'";
                break;
            case 'VARCHAR':
                $query = "ALTER TABLE {$tbname} ADD `{$value['name']}` VARCHAR({$value['length']}) NOT NULL DEFAULT '' COMMENT '{$value['comment']}'";
                break;
            case 'TEXT':
                $query = "ALTER TABLE {$tbname} ADD `{$value['name']}` TEXT({$value['length']}) NOT NULL DEFAULT '{$value['comment']}' COMMENT '{$value['comment']}'";
                break;
            case 'MEDIUMTEXT':
                $query = "ALTER TABLE {$tbname} ADD `{$value['name']}` MEDIUMTEXT NOT NULL DEFAULT '' COMMENT '{$value['comment']}'";
                break;
            case 'JSON':
                $query = "ALTER TABLE {$tbname} ADD `{$value['name']}` TEXT NOT NULL COMMENT '{$value['comment']}'";
                break;
