 ...
 * @LastEditTime: 2023-12-06 17:48:15
...
class ListInfo extends Base
{
    ...
    public function getList(int $isremove = 0, $page = 1, $keyword = '', $limit = 10)
    {
        ...
        if ($this->mod['type'] == 'classic') {
            ...
            if ($isremove) {
                ...
                $data['removeNum'] = $this->table::onlyTrashed()->where('category_id', $this->categoryid)->count();
            }
            $data['list'] = $res;
        }
    ...
    {
        $r = $this->table::withoutField('create_time,update_time')->find($id);
        $this->success('获取成功', $r);
    }
    ...
    {
        $dataList = $this->table::where('id', 'in', $ids)->select();
